...
//			}
	...
	#region Character APIs
		...
		public static bool DoesCharacterHaveAchievement(string charID, string achvID)
		{
			...
			{
				string check = achievements.FirstOrDefault((i)=> { return string.Equals(i, achvID);  });
				if(check != null)
				{
					return true;
				}
		...
		public static void GetCharacterStatistics()
		{
			...
			request.FunctionName = "GetCharacterStatistics";
			request.FunctionParameter = new { CharacterId = characterIDs };
		...
		public static void UpdateCharacterStatistics(string charID, Dictionary<string, int> updates)
		{
			...
			if(activeStats != null)
			{
				...
				ExecuteCloudScriptRequest request = new ExecuteCloudScriptRequest();
				request.FunctionName = "UpdateCharacterStats";
				request.FunctionParameter = new { CharacterId = charID,  CharacterStatistics = activeStats };
		...
		public static void OnGetCharacterInventorySuccess(GetCharacterInventoryResult result)
		{
	...
	#region misc
		public static void RedeemItemOffer(CatalogItem offer, string instanceToRemove, UnityAction<string> callback = null,  bool onlyRemoveInstance = false)
		{
			if(onlyRemoveInstance == true)
			{
				...
				request.FunctionParameter = new { PFID = PlayFabSettings.PlayerId, InstanceToRemove = instanceToRemove };
			...
			else
			{
				...
				removeReq.FunctionParameter = new { PFID = PlayFabSettings.PlayerId, InstanceToRemove = instanceToRemove };
				...
				awardRequest.FunctionParameter = new { PFID = PlayFabSettings.PlayerId, Offer = offer, SingleUse = offer.Tags.IndexOf("SingleUse") > -1 ? true : false};
